...
<div class="footer">
  <p><b>Registered Buyer Selling Payment Summary</b>(<span style="color:yellow;">{{$req_month}}</span>)</p>
</div>
<br>
<div class="footer2">
...
<br>
<div class="footer2">
  <p><b>Buyer's Wise Payment Summary</b></p>
</div>
<table id="customers">
  <tr>
    <th width="5%">SN</th>
  ...
  @foreach($payment_buyers as $key => $payment)
  <tr>
    <td>{{ $key+1 }}</td>
    <td>{{ $payment->user_id }}</td>
    <td>{{ $payment->name }}</td>
    <td>Rs. {{ number_format($payment->pay , 2) }}</td>
  </tr>
  @endforeach
</table>
<br> <br>
<div class="footer1">
  <p>{{date("Y F", strtotime($req_month))}} Collection Centre Total Payment Reciced <span style="color:yellow;font-size: 25px;font-weight: bold;">Rs. {{ number_format($total_payments , 2)}}</span></p>
</div>
